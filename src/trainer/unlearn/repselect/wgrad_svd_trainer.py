# python src/train.py --config-name=unlearn.yaml experiment=unlearn/wmdp_low_mi/default trainer=WGradSVD task_name=SAMPLE_UNLEARN
import logging
import math
...
class WGradSVD(UnlearnTrainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        # Pass B: retain-side weight-gradient accumulation into grad_acc
        ...
        # Pass D: forget-side backward + collapse + optimizer step
        model.zero_grad(set_to_none=True)
        with require_grad(self.base_trainable_params):
        ...
        if is_warmup_batch:
            # epoch 1: skip optimizer update by zeroing out grads of trainable params
    ...
    def collapse_hook(self, module, grad_input, grad_output):
        ...
        acts = module.wgrad_collapser.collapse_vecs(acts, "acts")
        grads = module.wgrad_collapser.collapse_vecs(grads, "grads")
