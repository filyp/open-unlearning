...
class RepSelect(UnlearnTrainer):
    def __init__(self, cfg, *args, **kwargs):
        ...
        self.use_lora = False
        self.collect_retain_acts = False
        self.collect_retain_grads = False
        self.batch_idx = 0
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        # Pass C.5: retain pass for top-token collapse (acts and/or grads)
        top_act = self.cfg.get("top_collapse", False)
        top_grad = self.cfg.get("top_grad_collapse", False)
        if top_act or top_grad:
            self.collect_retain_acts = top_act
            if top_grad:
                model.zero_grad(set_to_none=True)
                self.collect_retain_grads = True
                with require_grad(self.base_trainable_params):
                    output = model(**prep_batch(r_batch, model.device))
                    (-output.loss).backward()
                self.collect_retain_grads = False
    ...
    def collapse_hook(self, module, grad_input, grad_output):
        ...
        # Top-token collapse: project out matched retain activation/gradient directions
        # per (sample, forget-token) BEFORE flattening/distribution collapse.
        if self.do_collapse and self.cfg.get("top_collapse", False):
            retain_acts = module.retain_acts
            module.retain_acts = None
            retain_mask = retain_acts.norm(dim=-1) > 0  # (B, T_r)
            acts = self._top1_collapse(acts, retain_acts, retain_mask, eps=1e-12)
        if self.do_collapse and self.cfg.get("top_grad_collapse", False):
            retain_grads = module.retain_grads
            module.retain_grads = None
            retain_mask = retain_grads.norm(dim=-1) > 0  # (B, T_r)
            grads = self._top1_collapse(grads, retain_grads, retain_mask, eps=1e-12)
